...
import { goToEmojiPage, postEmojiRemove } from "./libs/index.mjs";

export const remover = async ({ inputs, history, nsfwAdded }) => {
  ...
  const _remove = async (inputs) => {
    ...
    try {
      ...
      while (i < decomojiListLength) {
        ...
        let res;
        try {
          res = await postEmojiRemove(page, WORKSPACE, name);
        } catch (error) {
