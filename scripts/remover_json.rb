# Decomoji Remover
class Remover
  DEFAULT_REMOVE_IMG_JSON = File.expand_path(File.dirname(__FILE__)) + "/list_v4.json"

  def initialize(remove_img_json: nil)
    @page = nil
    @agent = Mechanize.new
    @remove_img_json = remove_img_json || DEFAULT_REMOVE_IMG_JSON
  end
  ...
  def remove_decomojis
    files = nil
    emojis = list_emojis
    File.open(DEFAULT_REMOVE_IMG_JSON) do |file|
